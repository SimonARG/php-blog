<div class="create-post">
  <form enctype="multipart/form-data" class="new-post-form" action="/post/store" method="POST">
    ...
    <input minlength="4" maxlength="40" required type="text" id="title" name="title" <?php if (isset($errors['title_error'])) : ?> <?= "placeholder='" . $errors['title_error'] . "'" ?> <?= "class='title ph-error'" ?> <?php else : ?> <?php if (isset($errors)) : ?> <?= "value='" . $request['title'] . "'" ?> <?= "class='title'" ?> <?php else : ?> <?= "placeholder='Titulo'" ?> <?= "class='title'" ?> <?php endif; ?> <?php endif; ?>>
